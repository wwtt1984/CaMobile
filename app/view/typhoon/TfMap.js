    ...
    setForecastPath: function(tfbh,ybt,color,a){
        ...
        if(store.getCount()>0){
            ...
            for (var i = 0; i < store.getCount(); i++) {
                ...
                time.push("预报台:" + record.get('ybt') + "<br>时间:" + record.get('sj') + "<br>预报时间:" + record.get('ybsj') + "<br>经度:" + record.get('jd') + "<br>纬度" + record.get('wd'));
...
//        	makerarr.push(marker);
            ...
            switch(a){
                case 0:
                    me.flightPath1 = new google.maps.Polyline({
                    ...
                    me.flightPath1.setMap(me.map);
                    break;
                case 1:
                    me.flightPath2 = new google.maps.Polyline({
                        path: flightPlanCoordinates,
                        strokeColor: color,
                        icons:[{
                            icon: symbolOne,
                    ...
                    me.flightPath2.setMap(me.map);
                    break;
                case 2:
                    me.flightPath3 = new google.maps.Polyline({
                        path: flightPlanCoordinates,
                        strokeColor: color,
                        icons:[{
                            icon: symbolOne,
                    ...
                    me.flightPath3.setMap(me.map);
                    break;
                case 3:
                    me.flightPath4 = new google.maps.Polyline({
                        path: flightPlanCoordinates,
                        strokeColor: color,
                        icons:[{
                            icon: symbolOne,
                    ...
                    me.flightPath4.setMap(me.map);
                    break;
                case 4:
                    me.flightPath5 = new google.maps.Polyline({
                        path: flightPlanCoordinates,
                        strokeColor: color,
                        icons:[{
                            icon: symbolOne,
...
//半屏提示功能
    infoLastTfvalue: function(toZhuJiDistance) {
        ...
        if (this.store.getCount() > 0) {
            ...
            this.view = Ext.create('YzMobile.view.typhoon.PopUp');
...
//计算当前台风中心点到诸暨的距离.
    changelatlng: function() {
        ...
        var loc2 = new google.maps.LatLng(YzMobile.app.zhujimapCenter[0], YzMobile.app.zhujimapCenter[1]);
...
//两坐标点之间的线路(虚线路径)
    route: function() {
        if (this.line != null && this.pointMarkerZhuJi != null) {
            ...
            this.map.setZoom(11);
            this.map.panTo(new google.maps.LatLng(YzMobile.app.zhujimapCenter[0], YzMobile.app.zhujimapCenter[1]));
            var lineCoordinates = [
                new google.maps.LatLng(this.tfwd, this.tfjd),
                new google.maps.LatLng(YzMobile.app.zhujimapCenter[0], YzMobile.app.zhujimapCenter[1])
            ];
            var lineSymbol = {
                path: 'M 0,-1 0,1',
                strokeOpacity: 1,
    ...
    pointMarker: function() {
        image = new google.maps.MarkerImage(
            ...
            this.pointMarkerZhuJi = new google.maps.Marker({
                position: new google.maps.LatLng(YzMobile.app.zhujimapCenter[0], YzMobile.app.zhujimapCenter[1]),
                map: this.map,
                visible: true,
                title: "该点位置为宁波鄞州",
                shadow: shadow,
